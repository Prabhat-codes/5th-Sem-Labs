...
int search(struct sym_table *tab,char *nam,int n)
{
    int i=0;
    for(i=0;i<n;i++)
    {
...
struct sym_table insert(int sno,char *lexn,char *dt,char *t,int s)
{
...
void printTable(struct sym_table *tab,int n)
{   
    ...
    for(int i=0;i<n;i++)
    {
        ...
        if(tab[i].size==-1){
            ...
            printf("%s %s NULL\n",tab[i].lexeme,tab[i].type);
        }
        else{
            printf("%s %s %d\n",tab[i].lexeme,tab[i].dtype,tab[i].size);
        }
    }
}
...
int main()
{
  ...
  fclose(fa);
    fclose(fb);
    remove("temp.c");
    FILE *f1=fopen("sampleoutq1l1.c","r");
  ...
  if(f1==NULL)
    ...
    struct token tkn;
    struct sym_table st[10][100];
    int flag=0,i=0,j=0;
    int tabsizeOfToken[10];
    char w[25];
    w[0]='\0';
  ...
  while((tkn=getNextToken(f1)).row!=-1)
    {
      ...
      if((strcmp(tkn.type,"LB"))==0)
            {
                if(search(st[i],w,j)==0)
                {
                    ind++;
                    st[i][j++]=insert(ind,w,dbuf,"FUNC",-1);
                }
            }
      ...
      if((strcmp(tkn.type,"LS"))==0)
            {
                if(search(st[i],w,j)==0)
                {
                   ...
                   printf("<%s, %d, %d>\n",tkn.lexeme,tkn.row,tkn.col);
                    ...
                    ind++;
                    st[i][j++]=insert(ind,w,dbuf,"id",sizeOfToken(dbuf)*s);
                }
            }
      ...
      else
            {
                ...
                {
                    ind++;
                    st[i][j++]=insert(ind,w,dbuf,"id",sizeOfToken(dbuf));
                }
            }
        }
  ...
  int k=0;
  printf("Symbol Table\n");
  printf("Lexeme Name  Type  Size\n");
  for(k=0;k<i;k++)
    {
        printTable(st[k],tabsizeOfToken[k]);
